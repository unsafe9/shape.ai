...
The implemented POC uses a TypeScript Canvas2D renderer as a debuggable browser harness and provides a Rust/WASM scene core scaffold. The Rust core now exposes both a wgpu/WebGPU readiness probe and a visible WebGPU primitive renderer that uploads retained group/card primitives plus segmented cubic edge vertices and edge labels, consumes shared style token colors, renders grapheme-aware bitmap title/summary/edge-label glyphs through `unicode-segmentation` and `unicode-width`, reports CJK/fallback glyph counts, applies compact render patches, writes fixed card/edge/group vertex slots for group translate/card move/edit/select dirty ranges, rebuilds card draw order for z-order patches, uses spare group slots for group create/delete dirty writes, grows and compacts group slots, uses spare card slots for card create/delete dirty writes, grows and compacts card slots, uses spare edge slots for edge create/delete dirty writes, grows exhausted edge slots, compacts delete-heavy edge/free-slot churn, performs card/text/port/cubic-edge/group hit testing, submits render passes, presents frames, and reports GPU vertex/glyph/fallback-glyph/CJK-glyph/style-token/patch/dirty-write/rebuild/group-slot/group-grow/group-compact/card-slot/card-grow/card-compact/edge-slot/edge-grow/edge-compact counts. This is enough to prove the app boundary, retained scene contract, fixture scale, pointer batching, spatial index, cache stats, DOM edit overlay, benchmark path, group tag attach/filtering, comments/product export shell, copy/paste/duplicate/z-order app patch parity, first WebGPU lifecycle boundary, initial visible WebGPU canvas ownership, and first renderer-owned text snippet/style-token/hit-test/patch path without touching production canvas source. The remaining renderer-specific hardening is product-quality Rust/wgpu drawing: text shaping/cache and real font fallback beyond the bitmap atlas, richer card/edge styling fidelity, real Korean glyph rendering, and broader dirty-range updates behind the same API.
