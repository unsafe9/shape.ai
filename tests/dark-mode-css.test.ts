// AP4 (#12c) — full dark-mode surface inversion. The user reported the toggle only
// darkened "some floating UI"; this pins that EVERY shell surface inverts to a
// near-black palette in dark mode, asserting the REAL resolved CSS-variable graph
// (not a comment-satisfiable substring): the dark block must override the surface
// base/text/canvas to genuinely dark values that differ from light, and the key
// floating surfaces (toolbar, popups, modal, menus, panels) must consume that
// themeable base so they flip with it.
...
/** Extract `--name: value;` declarations from the FIRST matching `selector { … }`
 *  block. Good enough for the flat `:root` / `:root[data-theme="dark"]` blocks. */
function readVars(source: string, selector: string): Map<string, string> {
  ...
  // Strip block comments first so a `--var` that follows an inline comment in the
  // same `;`-delimited segment still parses (the comment text is not a declaration).
...
/** Resolve a var() expression against a variable map (one level of nesting is
 *  enough for `--surface: rgb(var(--surface-rgb) / a)`). */
...
describe("dark-mode CSS variable graph (AP4 #12c)", () => {
  it("flips the surface base from white to near-black (and they differ)", () => {
    expect(luminance("rgb(" + light.get("--surface-rgb")! + ")")).toBeGreaterThan(220); // ~white
    expect(luminance("rgb(" + dark.get("--surface-rgb")! + ")")).toBeLessThan(60); // near-black
    expect(dark.get("--surface-rgb")).not.toBe(light.get("--surface-rgb"));
  });

  it("flips text to light-on-dark (text luminance inverts)", () => {
    expect(luminance(light.get("--text")!)).toBeLessThan(80); // dark ink on light
    expect(luminance(dark.get("--text")!)).toBeGreaterThan(180); // light ink on dark
  });
  ...
  it("resolves the composed --surface token to a near-black surface in dark mode", () => {
    // --surface: rgb(var(--surface-rgb) / 0.82) — resolving the graph proves the
    // composed surface actually inherits the dark base (not just the raw triplet).
...
describe("every key floating surface consumes the themeable base (so it flips)", () => {
  // For each surface RULE, capture its `background:` and assert it resolves (through
  // the dark variable map) to a near-black color — i.e. it is wired to the theme,
  // not a hardcoded light value. Falsifiable: a surface that hardcodes white fails.
  ...
  for (const selector of [".toolbar-remote", ".toolbar-draw", ".settings-modal", ".node-context-menu", ".template-picker", ".color-popup"]) {
    it(`${selector} resolves to a dark surface in dark mode`, () => {
      ...
      // The surface may reference --surface (composed) or --surface-rgb / --surface-solid.
      ...
      // Take the first color token of the (possibly gradient) background.
