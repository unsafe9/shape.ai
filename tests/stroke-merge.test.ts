// Anchor-semantics v3 §4 follow-up — multi-stroke endpoint merge, driven
// through the SAME scene-core wasm the live app runs (contract tests, like
// freehand-recognition.test.ts). The shell's freehand release branches
// merge-FIRST: when `mergeOpenStrokeOps` returns ops, the stroke chains into
// the matched open-class object (edit-geometry batch on the survivor — NO
// insert-object, and the synthesizeReleaseAnchors path is skipped because the
// branch returns before it); when it returns null, the existing insert +
// release-anchoring path runs unchanged.
//
// The acceptance: a rect drawn in THREE strokes — left vertical, then an
...
describe("freehand release merges into a nearby open endpoint (merge-first branch)", () => {
  it("a stroke released ON an open object's endpoint authors an edit-geometry batch — no insert, anchors skipped", () => {
    // Stroke 1 already committed: the left vertical (300,200)->(300,100).
    ...
    // Stroke 2: the ㄱ (top + right), starting exactly on the vertical's top end.
    const gamma = [
    ...
    // The shell branch returns on non-null ops BEFORE freehandToObject /
    // synthesizeReleaseAnchors run: nothing inserts, no release anchor authors.
  ...
  it("THE acceptance: three strokes — vertical, ㄱ, bottom bar — close into ONE rect object", () => {
    ...
    // Stroke 3: the bottom bar lands on BOTH endpoints of the chained ⊓.
    ...
    // One object, closed, and Basic re-recognition snapped it to THE rect.
  ...
  it("a release away from every open endpoint returns null — the existing insert + anchor path runs", () => {
    const left = core.freehandToObject(strokeAlong({ x: 300, y: 200 }, { x: 300, y: 100 }), PEN.color, PEN.widthPx, "draw-1", "a1", "basic");
    // A closed target whose outline the stroke ends on: closed-class is never
    // a merge candidate, but it IS a release-anchor target.
