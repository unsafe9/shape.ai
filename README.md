...
Charrette is a local design-decision workspace that agents can read, edit, and extend through MCP. It stores typed design graphs on disk, exposes a React canvas for humans, and provides a stdio MCP server for AI agents that need to inspect or update the same local state.
...
- `CHARRETTE_HOST`: backend host, default `127.0.0.1`
- `CHARRETTE_PORT`: backend port, default `8787`
- `CHARRETTE_CLIENT_PORT`: Vite client port, default `5173`
- `CHARRETTE_DATA_DIR`: local storage root, default `.local`
- `CHARRETTE_REPO_ROOT`: repository root used when validating local export paths
...
```json
{
  "command": "npm",
  "args": ["run", "mcp"],
  "cwd": "/absolute/path/to/charrette"
}
```
...
- Charrette is local-first and file-backed.
