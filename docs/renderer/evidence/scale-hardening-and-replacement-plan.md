...
- Spatial index: `buildSpatialIndex()` and `querySpatialIndex()` remain as tested TS utilities, but the WebGPU-only render path no longer uses them per frame.
- Viewport culling: Rust WebGPU computes a padded world viewport, culls group/card/edge slots, and submits merged draw ranges instead of drawing the full retained vertex buffer.
- Edge culling: edges render when their cubic route bounds intersect the padded viewport.
- Text cache: Rust caches text line layout and wrapped-line results by text, width, line count, and font metrics.
- Edge cache: cubic edge routes are cached by edge id.
- Boundary stats: scene load, resize, queued camera updates, patches, and benchmark calls increment `boundaryCalls`.
- Rust boundary stats: generated WASM core load, resize, scene sync, compact WebGPU patch sync, hit testing, coalesced camera/render calls, and frame calls increment `rustBoundaryCalls`.
...
- Visible WebGPU primitive renderer: generated WASM can own a visible WebGPU canvas, upload group/card primitives plus segmented cubic edge vertices, edge labels, and bitmap text glyph quads, consume shared style token colors, cull visible slots into merged draw ranges, wrap title/summary/edge-label text by grapheme/display width through `unicode-segmentation` and `unicode-width`, cache text line/wrap layout results, report visible object/drawn vertex/draw range/text-cache/CJK/fallback glyph counts, draw selected group/card/edge outlines, apply compact render patches without re-sending the whole scene snapshot, write fixed card/edge/group vertex slots for group translate/card move/edit/select dirty ranges, rebuild card draw order for z-order patches, use spare group slots for group create/delete dirty writes, grow group slot capacity by inserting transparent slots before edge/card segments, compact delete-heavy group/free-slot churn by copying used group slots and shifting edge/card segments into a smaller GPU buffer, use spare card slots for card create/delete dirty writes, grow card slot capacity by appending transparent slots, compact delete-heavy card/free-slot churn by copying used card slots into a smaller suffix, use spare edge slots for edge create/delete dirty writes, grow edge slot capacity by inserting transparent slots before the card draw segment, compact delete-heavy edge/free-slot churn by copying used edge and card segments into a smaller GPU buffer, perform Rust-owned card/text/port/cubic-edge/group hit testing, submit render passes, and present frames.
...
- The Rust core has a visible WebGPU primitive renderer that consumes style tokens, culls draw ranges, and wraps/caches bitmap text by grapheme/display width, but not the production-quality Rust/wgpu draw backend.
- Scene buffers, render passes, style-token colors, selected outlines, viewport draw-range culling, grapheme-aware cached bitmap text snippets and edge labels with CJK/fallback glyph metrics, cubic edge geometry, compact patch ingestion, fixed-slot group translate/card move/edit/select dirty writes, z-order rebuilds, group-slot create/delete dirty writes, dynamic group-slot growth/compaction, card-slot create/delete dirty writes, dynamic card-slot growth/compaction, spare-slot edge create/delete dirty writes, dynamic edge-slot growth, edge-slot compaction, and Rust WebGPU hit testing exist, but Swash/Cosmic Text-level real shaping, real Korean glyph rendering, richer product card/edge styling, broader style/layout dirty invalidation, DOM overlay alignment/IME verification, and browser interaction proof are not production-ready.
...
- Visual regression screenshots are not committed; use Browser captures during review.
...
1. Harden the visible Rust/wgpu primitive renderer into the product renderer: real shaping and font fallback beyond the current grapheme-aware cached bitmap atlas, richer card/edge styling, and broader style/layout dirty-range updates.
