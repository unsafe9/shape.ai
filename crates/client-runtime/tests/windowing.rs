//! Windowing math + re-subscribe decisions (region seed, camera-move re-aim,
//! debounce coalesce, unchanged no-op, whole-canvas drop) and the
//! clean-outbox-on-canvas-switch rule.
...
fn coalesces_a_rapid_pan_into_a_single_re_subscribe() {
    // The decision layer only emits on the final settled viewport: the
    // post-debounce decision is one subscribe to the final window.
...
fn switch_canvas_starts_the_new_canvas_with_a_clean_outbox() {
    ...
    // switchCanvas builds a fresh outbox per canvas (no cross-canvas replay).
...
fn buffers_ops_while_offline_and_replays_them_on_reconnect_to_converge() {
    ...
    // Reconnect welcome: reconcile replays both in localSeq order.
