//! The single portable on-disk export format every adapter exports to and
//! imports from, so a store dumped from one backend can be loaded into any other.
//!
//! A bundle is a sharded directory; records are assigned to shards by a stable
//! hash of their id, so shards are written/read concurrently (one rayon task
//! each) and the same logical store always produces the same bytes (per-shard
//! CRC pins stability):
//!
//! ```text
//! bundle.shapestore/
...
//! ```
//!
//! Shard binary layout (records within a shard are id-sorted for byte stability):
//!
//! ```text
...
//! ```
//!
//! Export and import are **streaming**: peak buffering is one record plus small
//! writer buffers, never `O(total)`. [`export_bundle`] / [`import_bundle`] are
//! snapshot-shaped wrappers over the same streaming core for the in-memory path.
...
/// Bytes copied per chunk when streaming shard bodies; bounds the resident
/// buffer to a fixed size regardless of shard size.
const COPY_CHUNK: usize = 16 * 1024;

/// Per-shard manifest entry: lets import discover and verify shards without
...
/// FNV-1a hash assigning records to shards. Inlined (not std `Hasher`, whose
/// output is not stable across builds) so shard assignment is reproducible.
...
/// the final byte order matches a one-shot CRC of the assembled file.
struct Crc32 {
    state: u32,
}
...
/// A length/count as the format's `u32`. A field at or beyond the 4 GiB cap is
/// a programming error, so this asserts rather than silently truncating.
fn field_len(n: usize) -> u32 {
    u32::try_from(n).expect("bundle field length exceeds u32 (4 GiB cap)")
}

/// Append one record's frame to `buf` (body framing only, no header).
...
/// The fixed shard header bytes for a record count.
...
/// Write records from a lazy, **id-sorted** cursor into a portable bundle at
/// `root` in bounded memory: each record routes straight to its shard's on-disk
/// body, then a chunked finalize pass (parallel across shards via rayon)
/// prepends headers and computes CRCs, keeping peak at `O(COPY_CHUNK *
/// parallelism)`. `shard_count` must be >= 1.
    ...
    I: IntoIterator<Item = Result<Record>>,
{
    ...
    // Not atomic: stale shards are cleared and new ones written in place, so a
    // failure partway can leave `root` partial. Export to a fresh path when the
    // existing bundle must survive failure.
    clear_stale_shards(root)?;

    // One buffered body writer per shard. id-sorted input means per-shard
    // filtering preserves id order, matching the snapshot layout byte-for-byte.
    ...
    for record in records {
        ...
        // Turn a >u32::MAX count into a clean error, not a silently truncated,
        // CRC-sealed corrupt shard.
...
/// Snapshot-shaped wrapper over [`export_stream`]; streams the snapshot's
/// id-sorted records, so its bundles are byte-identical to the cursor export.
...
/// Read a bundle at `root` and feed every record to `sink`, one shard at a time
/// decoded frame-by-frame in bounded memory (one record resident, never the
/// whole shard). Returns the manifest after a full integrity check (per-shard
/// CRC + record count, and total record count).
...
/// Stream one shard: verify its header, decode each frame into `sink`, and
/// verify the per-shard CRC. Peak memory is one record.
...
/// Snapshot-shaped wrapper over [`import_stream`]; collects the streamed
...
/// Which shard an id maps to. Public, stable mapping so adapters can target a
/// single shard for per-record I/O.
...
/// Decode one shard fully into its id-sorted records, verifying its CRC.
/// Bounded by one shard's size.
...
/// rewrite just that shard and patch the manifest. The vec is re-sorted before
/// writing so byte-stability holds.
...
/// A lazy cursor over a bundle's records in **global id-sorted order** via a
/// streaming k-way merge across the id-sorted shards. Peak resident is
/// `O(shard_count)`; each shard's CRC is verified as it drains.
...
impl ShardStream {
    ...
    /// The next record, or `None` at the shard's end (after verifying CRC).
...
pub struct BundleCursor {
    ...
    /// Current front record per shard (`None` once that shard is drained).
...
/// A bounded, CRC-folding reader over a shard file, one frame at a time; never
/// materializes the whole shard.
...
/// Remove pre-existing `shard-*.bin` / stale `*.bin.body` files so a re-export
/// starts clean and a larger previous run cannot leak shards into a smaller one.
