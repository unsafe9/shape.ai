//! Chord-similarity deform for open-class paths. open-class <=> exactly one
//! subpath, `closed:false`. An open path's pose IS its endpoint pair: when the
//! endpoints move (s,e) -> (s',e'), the unique similarity
//! `S = T(s')·R(Δθ)·σI·T(−s)` taking the old chord to the new one is applied to
//! EVERY coordinate pair (bezier control points included), so the silhouette
//! rotates/stretches as one piece.
//!
//! [`deform_open_path`] is the single source both the commit path and the
//! renderer live preview call, so committed and previewed bytes cannot drift.
//!
//! Degenerate guard: an old chord under 1px, or σ outside `[1/SIGMA_MAX,
//! SIGMA_MAX]`, falls back to a pure translation by (s'−s).
//!
//! Pure (no time/rng/IO). Coordinates are object-local quantized units (Q=8), the
//! same space `reproject_node_local_quantized` emits.
...
/// True iff `geometry` is open-class: exactly one subpath, not closed. An
/// unhydrated geometry is classified from its path-string rather than reporting
/// false.
...
/// Path-string form of [`is_open_class`] (renderer-consumed). Malformed input is
/// not open-class.
...
/// Round a deformed coordinate to a quantized i32 unit (clamp into i32 range so
/// the narrowing is safe; NaN maps to 0). Shared (`pub(super)`) with the
/// multi-stroke merge's world->local rewrite.
...
/// `new_start`->`new_end` rewrites every coordinate pair (control points
/// included), round-quantized. `None` when `d` is not open-class.
pub fn deform_open_path(d: &str, new_start: (f64, f64), new_end: (f64, f64)) -> Option<String> {
    ...
    // The similarity as a complex ratio z = (e'−s')/(e−s): (zr, zi) encodes
    // R(Δθ)·σ, σ = |z|. `None` = degenerate => translation only.
    let mut z = None;
    ...
    let nodes = sub
        ...
        .map(|n| {
            ...
            // ABSOLUTE control points: every emitted C coord is round(S(absolute)).
...
/// True iff `t` is a pure translation: a moved open-class member whose endpoints
/// all follow keeps the 0-rebake SetTransform only under a pure translate.
...
/// The `(start_pinned, end_pinned)` flags of an open-class member of a moved set:
/// an endpoint anchored to a target OUTSIDE the moved set is pinned; one anchored
/// to a moved target follows the delta like a free endpoint (the delta cancels).
/// `None` for the legacy cases (fewer than two pairs, or an interior-node anchor).
...
/// How one open-class member of a moved batch commits (the commit cascade and the
/// renderer live preview both follow [`route_open_endpoints`]).
pub enum EndpointRoute {
    /// Nothing pins and the delta is a pure translate — keep the existing
    /// SetTransform (0-rebake).
    Translate,
    /// Both endpoints pinned by unmoved targets: the member cannot move, author
    /// nothing.
    Pinned,
    /// Rewrite the geometry to this endpoint pair via [`deform_open_path`].
    Deform { new_start: (f64, f64), new_end: (f64, f64) },
}

/// Route one open-class moved member: a followed endpoint maps through
/// `inv(T)·delta·T·p`; a pinned endpoint keeps its local position. `None` when `d`
/// is not open-class. Pin flags come from [`open_endpoint_pins`].
...
/// The commit ops of an endpoint-drag release: ONE chord-deform `edit-geometry`
/// moving the dragged endpoint (`node_index`, 0 or last) to `new_point_px` (world
/// px), plus a `set-anchor` rewrite of the owner's whole anchors vector — that
/// endpoint rebound to `snap` `(target_id, world_at_px)` on a snap, or removed in
/// empty space. `[]` when `id` is unknown, not open-class, or `node_index` is not
/// an endpoint.
pub fn endpoint_release_ops(
    ...
    // Drop this endpoint's binding, then rebind it when the release snapped onto a
    // live (non-self) target.
...
mod tests {
    ...
    /// A 3-node open path with bezier handles; chord (0,0)->(64,0), quantized
    /// units (8 = 1px).
    ...
    fn pure_translation_shifts_every_coordinate_pair() {
        // Both endpoints +(16,24): z = 1, so every node and absolute control point
        // shifts by that delta.
        assert_eq!(
    ...
    fn rotate_and_scale_carry_the_bezier_handles() {
        ...
        // S(x,y) = (−2y, 2x), applied to every absolute pair (control points
        // included).
    ...
    fn sub_pixel_chord_falls_back_to_translation() {
        // Old chord is 4 units (< 8 = 1px): rotation/scale ignored, the whole path
        // translates by (new_start − old_start) so the free end misses new_end.
        assert_eq!(
    ...
    fn sigma_outside_the_clamp_falls_back_to_translation() {
        // σ = 65 > 64: fallback; new_start == old_start, so d is unchanged.
        assert_eq!(
    ...
    fn interior_node_anchor_routes_to_legacy() {
        // Node 1 of a three-pair path is interior.
        let anchors =
    ...
    fn route_deforms_only_the_free_end_when_one_endpoint_pins() {
        // Body translate (40,30)px with the start pinned: the free end takes the
        // whole delta, the pinned end holds its local position.
        let delta = Transform3x3::translate(40.0, 30.0);
    ...
    fn route_deforms_under_a_non_translate_delta() {
        ...
        // (0,0)->(0,100)px even with nothing pinned.
    ...
    /// rect-a at (100,0), rect-b at (300,0), `edge` an identity-transform line
    /// whose node 1 anchors to rect-a, plus a 3-node `wire` for the interior guard.
    ...
    fn endpoint_release_rejects_non_endpoint_and_closed_class() {
        ...
        // An interior node is not this op's surface.
