// Committing a move of a target object must reproject the bound node of every object
// anchored to it so the anchored endpoint tracks the target's delta.
...
describe("anchorFollowOps (the commit path's follow ops)", () => {
  it("authors an edit-geometry that reprojects the endpoint of an object anchored to the moved target", () => {
    ...
    // The bound node sits at the target's new world point; the free node 0 is untouched.
  ...
  it("leaves an unrelated object untouched (it is not anchored to the moved target)", () => {
    const target = targetRect("rect-a", 200, 0);
    const line = anchoredLine(target);
    const unrelated = targetRect("rect-c", -300, -300);
  ...
  it("an Alt-created (unanchored) object does NOT move when its would-be target moves", () => {
    const target = targetRect("rect-a", 200, 0);
    // Snap bypassed (Alt-create), no anchor synthesized.
    const altLine = core.buildPrimitiveFromDrag("line", { start: { x: 40, y: 30 }, end: { x: 200, y: 30 } }, "edge-1", "a1");
  ...
  it("a move of an object that nothing is anchored to authors no follow ops (the no-op case)", () => {
    const target = targetRect("rect-a", 200, 0);
    const line = anchoredLine(target);
    // Move the LINE itself; nothing is anchored to it.
